<template>
  <UModal
    ...
    description="Berikan informasi detail mengenai masalah yang ditemukan."
    :ui="{
      content: 'sm:max-w-md',
      overlay: 'bg-black/40',
    ...
    <template #body>
      ...
      <div class="mb-4 space-y-1">
        <p class="text-sm text-neutral-600 leading-relaxed">
          Kami terus meningkatkan sistem informasi agar berjalan dengan baik untuk semua orang. Terima kasih atas feedback Anda.
        </p>
        <div>
          <a
            :href="state.url || 'https://simas.nusa.id/'"
            target="_blank"
            class="text-blue-500 font-medium underline text-sm break-all"
          >
            {{ state.url || 'https://simas.nusa.id/' }}
          </a>
        </div>
      </div>
      ...
      >
        ...
        <UFormField label="Deskripsi" name="description" required>
          <UTextarea
            ...
            placeholder="Jelaskan feedback Anda..."
            class="w-full"
            :rows="4"
          />
        </UFormField>
...
const schema = z.object({
  ...
  images: z.array(z.any()).min(1, 'Minimal 1 lampiran').max(3, 'Maksimal 3 lampiran')
})
...
watch(open, (isOpenVal) => {
  if (isOpenVal) {
    state.url = currentUrl.value || (typeof window !== 'undefined' ? window.location.href : '')
    state.description = ''
    state.type = 'issue'
